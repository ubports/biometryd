...
#
# - control
# - libbiometryd${soversion}.install.${target_arch}
#
...
full_version=$(cat "${version_dir}"/libbiometryd.full-version)
major_minor=$(cat "${version_dir}"/libbiometryd.major-minor-version)
soversion=$(cat "${version_dir}"/libbiometryd.soversion)
vivid_soversion=$(cat "${version_dir}"/libbiometryd.vivid-soversion)
...
warning_msg $infile
cat $warning $infile \
    | sed -e "s/@BIOMETRYD_SOVERSION@/${soversion}/" > "$outfile"

# Generate the install files, naming them according to the soversion.

# Install file for binary package
infile="${dir}"/libbiometryd.install.in
outfile="${dir}"/libbiometryd${soversion}.install
warning_msg "$infile"
